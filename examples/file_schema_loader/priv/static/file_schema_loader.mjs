...
var Spa = class {
  #runtime;
  constructor(root3, [init3, effects], update4, view4) {
    this.#runtime = new Runtime(root3, [init3, effects], view4, update4);
  }
  send(message) {
...
var start = ({ init: init3, update: update4, view: view4 }, selector, flags) => {
  ...
  return new Ok(new Spa(root3, init3(flags), update4, view4));
};

// build/dev/javascript/lustre/lustre.mjs
var App = class extends CustomType {
  constructor(init3, update4, view4, config2) {
    super();
    this.init = init3;
    this.update = update4;
    this.view = view4;
...
function application(init3, update4, view4) {
  return new App(init3, update4, view4, new$6(empty_list));
}
...
var ConditionalRule = class extends CustomType {
  constructor(if_schema, then_schema, else_schema) {
    super();
    this.if_schema = if_schema;
    this.then_schema = then_schema;
    this.else_schema = else_schema;
  }
};
var JsonSchema = class extends CustomType {
  constructor(title, description, field_type, properties, required2, defs, conditionals, string_constraints, number_constraints) {
    super();
    this.title = title;
    this.description = description;
    this.field_type = field_type;
...
// build/dev/javascript/formosh/form/converter.mjs
...
function set_field_value(fields, name2, value2) {
  let json_value = field_value_to_json_value(value2);
  let $ = find2(fields, (f) => {
    return f[0] === name2;
  });
  if ($ instanceof Ok) {
...
function modify_array_item(value2, index4, modifier) {
  ...
  let updated = index_map(
    ...
    (item, i) => {
      let $ = i === index4;
      if ($) {
        return field_value_to_json_value(
          modifier(json_to_field_value_safe(item))
        );
      } else {
        return item;
      }
...
function compare_values(json_val, field_val) {
  if (field_val instanceof StringValue && json_val instanceof JsonString) {
    let s2 = field_val[0];
    let s1 = json_val[0];
    return s1 === s2;
  } else if (field_val instanceof NumberValue && json_val instanceof JsonNumber) {
    let n2 = field_val[0];
    let n1 = json_val[0];
    return n1 === n2;
  } else if (field_val instanceof IntegerValue && json_val instanceof JsonInteger) {
    let i2 = field_val[0];
    let i1 = json_val[0];
    return i1 === i2;
  } else if (field_val instanceof BooleanValue && json_val instanceof JsonBool) {
    let b2 = field_val[0];
    let b1 = json_val[0];
    return b1 === b2;
  } else if (field_val instanceof NullValue && json_val instanceof JsonNull) {
    return true;
  } else {
    return false;
  }
}
function check_property_match(prop_schema, field_value) {
  let $ = prop_schema.enum_values;
  if ($ instanceof Some) {
    let $1 = $[0];
    if ($1 instanceof Empty) {
      let expected_values = $1;
      return any(
        expected_values,
        (expected) => {
          return compare_values(expected, field_value);
        }
      );
    } else {
      let $2 = $1.tail;
      if ($2 instanceof Empty) {
        let expected_value = $1.head;
        return compare_values(expected_value, field_value);
      } else {
        let expected_values = $1;
        return any(
          expected_values,
...
function evaluate_condition(condition, form_values) {
...
function merge_schema_properties(base_schema, conditional_props) {
  ...
  if ($ instanceof Some) {
    ...
    return new JsonSchema(
      ...
      base_schema.number_constraints
    );
  } else {
    return base_schema;
  }
}
function apply_then_branch(schema, rule) {
  let $ = rule.then_schema;
  if ($ instanceof Some) {
    let then_props = $[0];
    return merge_schema_properties(schema, then_props);
  } else {
    return schema;
  }
}
function apply_else_branch(schema, rule) {
  let $ = rule.else_schema;
  if ($ instanceof Some) {
    let else_props = $[0];
    return merge_schema_properties(schema, else_props);
  } else {
    return schema;
  }
}
function resolve_conditional_schema(base_schema, form_values) {
  return fold(
    base_schema.conditionals,
    base_schema,
    (schema, rule) => {
      let $ = evaluate_condition(rule.if_schema, form_values);
      if ($) {
        return apply_then_branch(schema, rule);
      } else {
        return apply_else_branch(schema, rule);
      }
    }
  );
}
  ...
  if (root_value instanceof ObjectValue) {
    ...
    return fold(
      ...
      (acc, field2) => {
        ...
        key = field2[0];
        json_val = field2[1];
        let $ = json_to_field_value(json_val);
        if ($ instanceof Some) {
          let field_val = $[0];
  ...
  if (msg instanceof UpdateFieldPath) {
    ...
    if ($) {
      let submitting_model = new FormModel(
        ...
        validated_model.resolved_schema,
...
function extract_string_value(value2) {
  if (value2 instanceof Some) {
    let $ = value2[0];
    if ($ instanceof StringValue) {
      let s = $[0];
      return s;
    } else if ($ instanceof NumberValue) {
      let n = $[0];
      return float_to_string(n);
    } else if ($ instanceof IntegerValue) {
      let i = $[0];
      return to_string(i);
    } else if ($ instanceof BooleanValue) {
      let $1 = $[0];
      if ($1) {
        return "true";
      } else {
...
function extract_number_value(value2) {
  if (value2 instanceof Some) {
    let $ = value2[0];
    if ($ instanceof NumberValue) {
      let n = $[0];
      return float_to_string(n);
    } else if ($ instanceof IntegerValue) {
      let i = $[0];
...
function extract_boolean_value(value2) {
  if (value2 instanceof Some) {
    let $ = value2[0];
...
function from_config(config2) {
  return create_form_with_config(config2);
}
function from_schema(schema) {
  let default_config = config(schema);
  return from_config(default_config);
}
function from_json_string(json_string) {
  echo("parser", void 0, "src/formosh.gleam", 223);
  let $ = parse_schema(json_string);
  if ($ instanceof Ok) {
    let schema = $[0];
    return new Ok(from_schema(schema));
  } else {
    return $;
  }
}
...
// build/dev/javascript/file_schema_loader/fetch_schema.mjs
...
var Model = class extends CustomType {
  constructor(selected_schema, schema_content, form_html, available_schemas, error) {
    super();
    this.selected_schema = selected_schema;
    this.schema_content = schema_content;
    this.form_html = form_html;
    this.available_schemas = available_schemas;
    this.error = error;
  }
};
var LoadSchema = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
var SchemaFetched = class extends CustomType {
  constructor($0, $1) {
    super();
    this[0] = $0;
    this[1] = $1;
...
function init2(_) {
  let schemas = toList(["contact_form.json"]);
  return [
    new Model(
      new None(),
      new None(),
...
function get_display_name(filename) {
  let _pipe = filename;
  let _pipe$1 = replace(_pipe, ".json", "");
  let _pipe$2 = replace(_pipe$1, "_", " ");
  let _pipe$3 = replace(_pipe$2, "-", " ");
  return capitalise(_pipe$3);
}
function view3(model) {
  return div(
    toList([class$("container")]),
    toList([
      h1(toList([]), toList([text3("JSON Schema File Loader")])),
      div(
        toList([class$("schema-selector")]),
        toList([
          h2(toList([]), toList([text3("Select a schema:")])),
          div(
            toList([class$("schema-list")]),
            map(
              model.available_schemas,
              (filename) => {
                let _block;
                let $ = model.selected_schema;
                if ($ instanceof Some) {
                  let selected2 = $[0];
                  _block = selected2 === filename;
                } else {
                  _block = false;
                }
                ...
                return button(
                  toList([
                    on_click(new LoadSchema(filename)),
                    class$(
                      (() => {
                        if (is_selected) {
                          return "schema-button selected";
                        } else {
                  ...
                  toList([
                    text3(get_display_name(filename)),
                    (() => {
                      if (is_selected) {
                        return span(
                          toList([class$("badge")]),
      ...
      (() => {
        ...
        if ($ instanceof Some) {
          let form_info = $[0];
          return div(
            toList([class$("form-container")]),
            toList([
              h2(toList([]), toList([text3("Schema Information:")])),
              div(
                toList([class$("info-box")]),
                toList([
                  p(
                    toList([]),
                    toList([
                      text3(
                        "File: " + unwrap(model.selected_schema, "")
                      )
                    ])
                  )
                ])
              ...
              div(
                toList([class$("schema-preview")]),
                toList([pre(toList([]), toList([text3(form_info)]))])
              ),
              div(
                toList([class$("action-box")]),
                toList([
                  p(
                    toList([]),
                    toList([
                      text3("To see the full interactive form, run "),
                      code(toList([]), toList([text3("gleam run")])),
                      text3(" in the project root.")
                    ])
                  )
                ])
              )
          ...
          let $1 = model.selected_schema;
          if ($1 instanceof Some) {
            return none2();
          } else {
            return div(
              toList([class$("placeholder")]),
              toList([
                p(
                  toList([]),
                  toList([text3("Select a schema to load")])
                )
              ])
            );
          }
...
function generate_form_preview(json_content) {
  let $ = from_json_string(json_content);
  if ($ instanceof Ok) {
    return "Schema successfully parsed and can be used for form generation.\n\nFirst 500 characters of JSON schema:\n\n" + slice(
      json_content,
      0,
      500
    );
...
function fetch_schema_effect(filename) {
  return from(
    (dispatch) => {
      ...
      return fetchSchema(
        url,
        (result) => {
          let $ = starts_with(result, "Error:");
          if ($) {
            echo2("ERROR", void 0, "src/file_schema_loader.gleam", 218);
            return dispatch(new SchemaFetched(filename, new Error2(result)));
          } else {
            return dispatch(new SchemaFetched(filename, new Ok(result)));
          }
        }
      );
    }
...
function update3(model, msg) {
  if (msg instanceof LoadSchema) {
    ...
    if (result instanceof Ok) {
      ...
      echo2("Get content", void 0, "src/file_schema_loader.gleam", 65);
      let $ = from_json_string(content);
      if ($ instanceof Ok) {
        return [
          new Model(
            new Some(filename),
            new Some(content),
            new Some(generate_form_preview(content)),
            model.available_schemas,
            new None()
          ),
          none()
        ...
        return [
          new Model(
            ...
            new None(),
            model.available_schemas,
            new Some("Error parsing JSON schema")
          ),
          none()
        ];
      }
      ...
      let err = result[0];
      echo2("Get error " + err, void 0, "src/file_schema_loader.gleam", 94);
      return [
        new Model(
          new Some(filename),
          new None(),
          new None(),
          model.available_schemas,
          new Some("Error loading schema: " + err)
        ),
        none()
      ];
    }
...
function main() {
  let app = application(init2, update3, view3);
  ...
  if (!($ instanceof Ok)) {
    throw makeError(
      ...
      { value: $, start: 593, end: 642, pattern_start: 604, pattern_end: 609 }
    );
  }
  return void 0;
}
