...
1. **Tokens** — `--formosh-*` custom properties inherit across the shadow boundary ([full list](docs/guides/styling.md#tokens)):
...
3. **Adopted selectors** — Lustre copies the page's stylesheets into the shadow root once, right after the element is created, so plain class and `[part=…]` selectors apply inside, field state included:
...
formosh ships no look of its own. Its only defaults are the ones features cannot work without — a `ui:layout` `Row`'s grid and a collapsing array row's fold — and the only inline styles it writes are the swipe widget's runtime drag and fly-off transforms; `docs/guides/styling.md` lists them.
