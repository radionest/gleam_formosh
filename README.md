...
- **Plain Lustre app** (`lustre.start`, as in the Quick Start): there is no shadow root, so `::part()` does not apply — use class and `[part=…][data-…]` selectors, set tokens on any ancestor (`:root`, the mount point), and declare `@layer formosh, …;` first in your CSS, or formosh's defaults outrank the layers your stylesheets declared before the form mounted. See `docs/guides/styling.md#plain-lustre-app-no-shadow-root`.
