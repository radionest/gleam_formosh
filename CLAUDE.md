...
- `allOf` composition is merged at parse time (`schema/composer.gleam`): members fold in array order, parent-local keywords win last, same-key properties deep-merge field-by-field, bounds combine stricter-wins, scalars (enum/oneOf/pattern/format) take the later value wholesale — no intersection, member conditionals are lifted to the parent. Members parse strictly: a boolean `true` member is a skipped no-op, `false` or a malformed member is a `ParseError` (unlike `oneOf` extraction, which stays lenient). Post-parse `all_of` is always `None` outside `$defs` (defs entries stay raw source material; an `allOf` inside `$defs` is dropped on serializer round-trip). `oneOf`/`anyOf` composition keywords are not yet implemented
