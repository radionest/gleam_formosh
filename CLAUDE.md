...
- **Array reconcile (`defaults.ensure_min_items`)**: runs after every values recompute that precedes validation — init, reset, `UpdateFieldPath`, `ClearFieldPath`, `AddArrayItemPath`, `apply_answers`, component re-init — topping arrays up to `minItems` (checkbox-group arrays — `types.is_multi_select` — are skipped). Deliberately NOT run on `RemoveArrayItemPath`/`MoveArrayItemPath` or in `ValidateForm` (don't fight the user; validation reports under-min instead). New values-recompute paths must call it too
- **Array collapse state** (`array_collapse_off`, `array_rows_expanded`): view-only, path-keyed, never touches `values` or validation; reindexes with the array in the `RemoveArrayItemPath` / `MoveArrayItemPath` folds, same as `touched_fields`. `AddArrayItemPath` also force-expands the new row into `array_rows_expanded` so it's never born collapsed, and `clear_subtree` (the union-branch-switch reset) prunes both lists on the same prefix as `touched_fields` — a future array-mutating path must preserve all three invariants. Collapsing folds rather than unmounts: for a collapse-enabled array every row's fields are wrapped in `array-item-body` (`render_item_body`), rendered in both states; the `grid-template-rows: 1fr↔0fr` fold and its transition come from the library stylesheet (`internal/stylesheet.gleam`), keyed on the row's `data-collapsed` — a wrapper appearing only once the row is shut would have nothing to animate from. Collapsed bodies are `inert`; arrays without `collapseCompleted` get no wrapper at all. A row's children are unkeyed, so the summary must occupy its slot in every state (`element.none()` when the row is incomplete): a conditionally-present summary shifts the header and body down one index, and the positional diff then rebuilds the body — a fresh element at `0fr` has nothing to transition from, so the automatic fold snaps and the focused input is torn down mid-edit. Markup is unaffected either way, so only the `completing a row folds the body node it already had` e2e test catches a regression here
- **Circular $refs**: Resolver maintains visited set — do not bypass it
- **View refactors**: Required-asterisk (` *` with `class="formosh-required"`) and error wrappers are easy to drop when unifying renderers. Use `field_common.render_required_marker` and let the field dispatcher own error display — re-render the marker explicitly per container, not «implicitly via the parent»
...
Styling: the component runs in open Shadow DOM, so target it via `::part()` selectors (`formosh-form::part(input)`, etc.) and `[part=field][data-error]` / `[part=field][data-readonly]` for field state, plus `[part=field][data-name=…]` / `[data-path=…]` for field identity (not state). Parent stylesheets are also auto-adopted, so plain `.formosh-input { ... }` rules still apply inside the shadow root. Part catalog: `docs/guides/styling.md`. formosh's own defaults (the `ui:layout` Row grid, the array fold) live in `src/formosh/internal/stylesheet.gleam`, rendered as the container's first child inside `@layer formosh` — the lowest layer, so consumer rules override them without `!important`, with two exceptions documented in `docs/guides/styling.md`: in a plain Lustre app, page layers declared ahead of the form need `@layer formosh` declared first; and layers in a sheet Lustre must clone as a `<link>` (cross-origin without CORS, or using `@import`) rank below `formosh`. Inline `style` is reserved for runtime state (the swipe widget's drag/fly-off); never add an inline default. Tokens: `--formosh-row-gap`, `--formosh-row-min`, `--formosh-collapse-duration`. Never write an exact-match `[part="…"]` selector in that stylesheet — render tests detect elements by the literal `part="…"` string. Scope every selector there as `:where(.formosh-container …)` — in a plain Lustre app that `<style>` is a page stylesheet (which Lustre also copies into other components' shadow roots), so an unscoped part selector leaks.
...
- All `.mjs` files are auto-generated — do not read or edit them (except `src/formosh/ffi/*_ffi.mjs` which are hand-written FFI, and the hand-written E2E tooling under `e2e/*.mjs`)
- `allOf` composition is merged at parse time (`schema/composer.gleam`): members fold in array order, parent-local keywords win last, same-key properties deep-merge field-by-field, bounds combine stricter-wins, scalars (enum/oneOf/pattern/format) take the later value wholesale, member conditionals are lifted to the parent. The document root parses as a `SchemaProperty` and converts to `JsonSchema` post-merge, so a member can type a typeless root (the `object` default applies only when composition supplies no type) and a root-level `$ref` resolves. Strictness: `false` or malformed members, disjoint `type` declarations (`number`∧`integer` narrows to `integer`), and min/max pairs that cross in a composed node's merged constraints (even bounds authored on the node itself beside a non-empty `allOf`) are `ParseError`s (`UnsatisfiableSchema` with a `#/path` breadcrumb) — a composition that validates nothing is rejected instead of rendered; `true` members and empty `allOf` are no-ops, and single-schema (no-`allOf`) parsing keeps its lenient #63 normalization. Post-parse `all_of` is always `None` outside `$defs` (defs entries stay raw source material; an `allOf` inside `$defs` is dropped on serializer round-trip)
- `anyOf` is parsed, `$ref`-resolved inside members, and normalized at parse time by `composer.normalize_any_of`: null members drop into `nullable` (an empty nullable field validates as satisfied, submits `null`, and never shows the required asterisk — even when the field is named in `required`, Pydantic `Optional[X]`-style); a single surviving non-null member merges into the node itself (`Optional[X]` renders as a plain `X` field); 2+ surviving members stay in `any_of` and render as a runtime branch chooser (`model.selected_branches`, `SelectUnionBranchPath` msg, radio ≤5 branches / select >5, `ui:widget: "select"|"radio"` override, parts `union`/`union-radio`/`union-select`) — switching to a genuinely different branch clears the subtree (an array row is reset in place rather than removed, so sibling indices never shift) and applies the new branch's own defaults, scoped to that field's subtree only (unrelated fields elsewhere in the form are never re-hydrated). Re-selecting the branch already active — stored or currently active by inference — is a no-op: values/errors/touched stay untouched (a checked radio's `on_click` fires even without a change). Branch inference (no stored selection: type match for scalars, property-key overlap for objects, else 0) is resolution-time only for programmatic paths — init, `apply_answers`, component re-init all infer fresh and never persist — but a user-driven edit (`UpdateFieldPath`/`ClearFieldPath`) inside an inferred branch persists that inference on first edit, so clearing the field that drove it doesn't snap the chooser back to branch 0. Member extraction is lenient (malformed members are dropped); a parent `type` disjoint from every surviving member's type is a `ParseError` (`UnsatisfiableSchema`), same strictness as `allOf`. `oneOf` remains const+title enum rendering only — not processed as a schema-variant selector
...
- An array with `uniqueItems: true` whose scalar `items` carry options (`enum` of 2+ values / `oneOf` const+title) renders as a checkbox group (`types.is_multi_select` → `string_field.render_checkboxes`): typed consts in schema order, unchecking the last box removes the key, `maxItems` disables the remaining boxes, no `minItems` top-up. `uniqueItems` is also validated on every array. See `docs/reference/widgets.md#checkbox-group-multi-select`
- `x-widget: "hidden"` / `ui:widget: "hidden"` suppresses rendering at any depth; the field still validates and submits. Combine with JSON Schema `default` for `<input type="hidden">`-like behaviour. The default submit gate (`model.can_submit`) stays **strict** — a required hidden field without a satisfying value blocks submit. To surface the otherwise-invisible cause, a `console.warn` (via `ffi/console`) lists the suppressed-path errors whenever they are the *only* blockers. It is emitted from `component_core` at form (re)initialisation — the submit button is `disabled` in this state, so `FormSubmit` never fires through `<formosh-form>` — via `component_core.hidden_blocks_warn` wrapping `update.warn_only_hidden_blocks_effect`; `update.FormSubmit` emits the same warn for headless `model.init` embeddings that have no disabled-button gate. These are the only warn sites — entering the state mid-session (a value change flipping a conditional branch, or a dynamic `UiSchemaChanged`) does not emit it; tracked in #65. Visible errors keep the warn silent since the UI already explains the block. Callers that want a permissive gate (submit succeeds despite invisible required errors — e.g. the backend supplies the value) can read `model.is_valid_for_submit`. `model.hidden_errors` returns the suppressed slice for custom diagnostics. The same suppression set covers `readOnly` fields when `show_readonly_fields: false`. Walker lives in `formosh/form/visibility`; called on demand from `model.invisible_paths` so no `FormModel` field needs to track it
