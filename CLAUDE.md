...
- **Array collapse state** (`array_collapse_off`, `array_rows_expanded`): view-only, path-keyed, never touches `values` or validation; reindexes with the array in the `RemoveArrayItemPath` / `MoveArrayItemPath` folds, same as `touched_fields`
