...
- `allOf` composition is merged at parse time (`schema/composer.gleam`): members fold in array order, parent-local keywords win last, same-key properties deep-merge field-by-field, bounds combine stricter-wins, member conditionals are lifted to the parent. Post-parse `all_of` is always `None` outside `$defs` (defs entries stay raw source material). `oneOf`/`anyOf` composition keywords are not yet implemented
