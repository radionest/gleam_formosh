...
- `x-widget: "hidden"` suppresses rendering at any depth; the field still validates and submits. Combine with JSON Schema `default` for `<input type="hidden">`-like behaviour. A required hidden field without `default` will block submit — validation runs as for any other field. The same gotcha applies recursively: a required leaf inside a hidden object/array (without `default`) blocks submit even though the parent is suppressed. Filtering of those errors will land in a follow-up issue
