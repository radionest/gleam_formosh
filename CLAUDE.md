...
- **Array collapse state** (`array_collapse_off`, `array_rows_expanded`): view-only, path-keyed, never touches `values` or validation; reindexes with the array in the `RemoveArrayItemPath` / `MoveArrayItemPath` folds, same as `touched_fields`. `AddArrayItemPath` also force-expands the new row into `array_rows_expanded` so it's never born collapsed, and `clear_subtree` (the union-branch-switch reset) prunes both lists on the same prefix as `touched_fields` — a future array-mutating path must preserve all three invariants. Collapsing folds rather than unmounts: for a collapse-enabled array every row's fields are wrapped in `array-item-body` (`render_item_body`), rendered in both states and carrying the `grid-template-rows: 1fr↔0fr` transition as inline styles — a wrapper appearing only once the row is shut would have nothing to animate from. Collapsed bodies are `inert`; arrays without `collapseCompleted` get no wrapper and no inline styles at all. A row's children are unkeyed, so the summary must occupy its slot in every state (`element.none()` when the row is incomplete): a conditionally-present summary shifts the header and body down one index, and the positional diff then rebuilds the body — a fresh element at `0fr` has nothing to transition from, so the automatic fold snaps and the focused input is torn down mid-edit. Markup is unaffected either way, so only the `completing a row folds the body node it already had` e2e test catches a regression here
