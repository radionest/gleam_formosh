...
pub fn conditional_field_appears_when_condition_met_test() {
  ...
  let then_schema = SchemaProperty(
    ..empty_property(),
    properties: Some(dict.from_list([
      #("is_confidential", SchemaProperty(
        ..empty_property(),
  ...
  let conditional_rule = ConditionalRule(
  ...
  let form_values_met = dict.from_list([
    #("subject", StringValue("Общий вопрос")),
  ])
  
  let resolved_schema = conditional_resolver.resolve_conditional_schema(
    schema,
    form_values_met,
  )
  ...
  let form_values_not_met = dict.from_list([
    #("subject", StringValue("Техническая поддержка")),
  ])
  
  let resolved_schema_no_match = conditional_resolver.resolve_conditional_schema(
    schema,
    form_values_not_met,
  )
...
pub fn conditional_else_branch_test() {
  ...
  let else_schema = SchemaProperty(
    ..empty_property(),
    properties: Some(dict.from_list([
  ...
  let form_values_true = dict.from_list([
    #("hasAccount", BooleanValue(True)),
  ])
  
  let resolved_then = conditional_resolver.resolve_conditional_schema(
    schema,
    form_values_true,
  )
  ...
  let form_values_false = dict.from_list([
    #("hasAccount", BooleanValue(False)),
  ])
  
  let resolved_else = conditional_resolver.resolve_conditional_schema(
    schema,
    form_values_false,
  )
...
pub fn is_field_visible_test() {
  ...
  let base_properties = dict.from_list([
    #("subject", empty_property()),
    #("message", empty_property()),  // This field is always visible
  ])
  ...
  conditional_resolver.is_field_visible("special_field", schema, form_values_met)
  |> should.be_true()
  
  conditional_resolver.is_field_visible("special_field", schema, form_values_not_met)
  |> should.be_false()
}